...
class App {
  static init(): void {
    ...
    store.subscribe(() => {
      ...
      // if (token) {
      //   //как только получили токен положить все в стейт из юзер даты, потом запустить заводы либо в хэлперах signIn, остановить старые заводы, отправить данные если была включена другая сессия
